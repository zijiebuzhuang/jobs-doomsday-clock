/**
 * Test version of enrich-guidance.mjs that works on a 3-occupation sample.
 */
import { readFileSync, writeFileSync } from 'fs'
import { callLLM } from './fetch-news.mjs'

const GUIDANCE_PATH = 'public/occupation-guidance-sample.json'
const OUTPUT_PATH = 'public/occupation-guidance-sample-enriched.json'
...
async function main() {
  ...
  for (let i = 0; i < guidance.length; i++) {
    ...
    try {
      ...
      console.log(`  ${progress} ${entry.title}`)
