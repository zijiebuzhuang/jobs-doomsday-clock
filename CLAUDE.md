...
- **Push trigger rule:** `Daily News Fetch` triggers the relay when the boolean `inputs.send_push` is true, which is the workflow default. The official cron-job.org call may omit inputs or send `{"ref":"main","inputs":{"send_push":true}}`; manual recovery runs should explicitly set `send_push` to false if they should not notify users. When enabled, the workflow sends the same daily notification to both `production` and `sandbox` so TestFlight/App Store users and Xcode debug builds can both validate the notification copy.
